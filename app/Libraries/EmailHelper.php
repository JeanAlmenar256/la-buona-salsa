...
class EmailHelper
{
     ...
     */
    public static function enviarVerificacion(string $email, string $nombre, string $codigo, string $token): array
    {
        $link = base_url('verificar-email/' . $token);
...
<head>
    ...
    <style>
        ...
        .email-header h1 { margin: 0; font-size: 26px; letter-spacing: 1px; color: #ffffff; }
...
<body>
    <div class='email-container'>
        ...
        <div class='email-body'>
            <h2 style='color: #9B1212; margin-top: 0;'>¡Bienvenido/a, " . htmlspecialchars($nombre) . "!</h2>
            <p>Para proteger la seguridad de tu cuenta y confirmar que este correo es válido para tus pedidos y notificaciones de entrega, por favor ingresa el siguiente código de activación de 6 dígitos:</p>
            ...
            <div style='text-align: center;'>
                <a href='" . $link . "' class='btn-activate'>Activar mi Cuenta Ahora &rarr;</a>
            </div>

            <p style='font-size: 13px; color: #777; margin-top: 25px;'>
                Si el botón no funciona, copia y pega este enlace en tu navegador:<br>
                <a href='" . $link . "' style='color: #fd580f; word-break: break-all;'>" . $link . "</a>
            </p>
        </div>
        <div class='email-footer'>
            &copy; " . date('Y') . " La Buona Salsa. Este correo es una notificación automática de seguridad.
        </div>
    </div>
</body>
</html>";
        ...
        $hasSmtp = !empty($configEmail->SMTPHost) || !empty(getenv('SMTP_HOST'));

        $sent = false;
        $errorMsg = null;

        // Si hay un servidor SMTP configurado, realizamos el envío real
        if ($hasSmtp) {
            $emailService = \Config\Services::email();
            $emailService->setTo($email);
            $emailService->setFrom('no-reply@labuonasalsa.com.ar', 'La Buona Salsa');
            $emailService->setSubject("🔐 Código de Activación: $codigo - La Buona Salsa");
            ...
            // En desarrollo local sin SMTP configurado, registramos en log y completamos de forma instantánea
            log_message('info', "Email de verificación para $email: Código $codigo | Enlace: $link");
            $sent = true;
        }
